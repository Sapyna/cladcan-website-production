...
function Panel({active}){
  ...
  if(active==="benefits") return <div className={styles.benefitsPanel}><p className={styles.kicker}>WHY CHOOSE ACM</p><h2>Performance, fabrication freedom and a controlled architectural finish.</h2><div className={styles.benefitsLayout}><figure className={styles.benefitsVisual}><Image src="/images/exterior-systems/acm-acp/public-infrastructure-acm-panel-application.webp" alt="Architectural ACM panel façade application" fill sizes="(max-width: 900px) 100vw, 38vw" /><figcaption>Architectural ACM façade application</figcaption></figure><div className={styles.featureGrid}>{benefits.map(([title,text],i)=><article key={title}><span>{String(i+1).padStart(2,"0")}</span><div><h3>{title}</h3><p>{text}</p></div><Check size={18}/></article>)}</div></div></div>;
