...
export default function GoogleReviews(){
 ...
 return <div className={s.block}>
  <div className={s.summary}>
   <div className={s.scoreGroup}><span className={s.googleLabel}><b>G</b> Google Reviews</span><div className={s.scoreLine}><strong>{data.businessRating??"—"}</strong><div className={s.scoreMeta}><div className={s.stars}>{[1,2,3,4,5].map(i=><Star key={i} size={18} fill="currentColor"/>)}</div><span>Based on {data.businessReviewCount??"—"} reviews</span></div></div></div>
